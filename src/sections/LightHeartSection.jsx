...
import '../styles/lightHeartSection.css';

/* =========================================================
   YUKTIC ANIMATION CONFIG & DATA
========================================================= */
const STROKE_DURATION = 0.72;
const START_DELAY = 3.0; // Starts writing after 5 seconds
...
const STROKES = LETTERS.flatMap((paths, letterIndex) =>
  paths.map((path) => ({ path, x: letterIndex * 116 }))
);

const TOTAL_WRITING_DURATION = START_DELAY + STROKES.length * STROKE_DURATION;

function YukticWritingHeader() {
  return (
    <div
      className="yuktic-header-animation"
      style={{ "--writing-duration": `${TOTAL_WRITING_DURATION}s` }}
    >
      <svg
      ...
      >
        {STROKES.map(({ path, x }, index) => {
          const delayNum = START_DELAY + index * STROKE_DURATION;
          const delayStr = `${delayNum}s`;

          return (
            <g key={`${x}-${path}`} transform={`translate(${x} 0)`}>
              <path
...
const LightHeartSection = () => {
  ...
  return (
    <section className="candid-section">
      ...
      {/* Blue Faded Bottom Gradient */}
      <div className="section-bottom-blue-fade"></div>

      {/* Main Section Content */}
      <div className="section-content-container">
        
        {/* LEFT: Semi-Transparent Live View Card */}
        <div className="live-view-card">
              ...
              <div className="timeline-chart">
                <svg viewBox="0 0 160 48" className="sparkline-svg">
                  ...
                  <line
                    ...
                    strokeDasharray="3,3"
                  />
                  <circle cx="155" cy="24" r="4" fill="#38bdf8" />
                </svg>
              </div>
            </div>
          </div>
          ...
          <div className="teams-hub-section">
            <span className="teams-header-label">CONNECTED TEAMS & CAPABILITIES</span>
            <div className="hub-network-container">
              <svg className="hub-lines-svg" viewBox="0 0 300 220">
                <line x1="150" y1="110" x2="60" y2="45" className="hub-line" />
                <line x1="150" y1="110" x2="240" y2="45" className="hub-line" />
                <line x1="150" y1="110" x2="60" y2="175" className="hub-line" />
                <line x1="150" y1="110" x2="240" y2="175" className="hub-line" />
              </svg>

              {/* Central Green Clover Hub Node */}
              <div className="hub-center-node" title="Yuktic Core">
        ...
        {/* RIGHT: Typography + Horizontal Compact Service Grid */}
        <div className="text-content">
          <YukticWritingHeader />

          <div
            className={`card-text-wrapper ${
              initialLoaded ? "initial-ready" : "initial-waiting"
            }`}
            key={currentItem.id}
          >
            {/* Modern Glow Chip */}
            ...
            <h1 className="service-title drop-down-anim">{currentItem.title}</h1>
            <p className="service-description drop-down-anim-delay">
              {currentItem.desc}
            </p>

            {/* Same-Line Horizontal Service Selector Cards */}
              ...
              {servicesData.map((item, idx) => {
                ...
                return (
                  <div
                    key={item.id}
                    onClick={() => setActiveService(idx)}
                    className={`glass-service-card ${isActive ? "active" : ""}`}
                  >
